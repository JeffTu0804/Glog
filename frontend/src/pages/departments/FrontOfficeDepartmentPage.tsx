...
const HANDLE_ROLES: UserRole[] = ["ADMIN", "FRONT_DESK"];

export function FrontOfficeDepartmentPage() {
  ...
  const filtered = useMemo(
    () =>
      requests.filter(
        (r) => isDepartmentTask(r) && r.targetDepartment === "FRONT_DESK",
      ),
    [requests],
  );
  ...
  async function load() {
    ...
    try {
      ...
      const { requests: list } = await api.getServiceRequests(token, apiView, "FRONT_DESK");
      setRequests(list);
    } catch (err) {
  ...
  return (
    <div>
      <PageHeader
        title="客務部"
        subtitle="客人協調任務 — 換房、客訴跟進、通知客人等，接單後上傳照片結案"
        accent="sky"
      />
